...
function autocomplete(input, latInput, lngInput) {
    ...
    //if someone hits enter on the address field then dont submit the form.
...
//function to make the search results into HTML.
...
// actual function to search for the stores.
function typeAhead(search) {
  ...
  searchInput.on("input", function () {
    ...
    //if the search value  dosent exist then hide the results
    if (!this.value) {
    ...
    axios.get("/api/search?q=" + this.value).then(function (res) {
      ...
      // show something if the store results are not really found yet.
  ...
  searchInput.on("keyup", function (e) {
    /* All we care about is 13 for enter and 40 for down and 38 for up */
...
function loadPlaces(map) {
  var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 43.2557;
  var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : -79.8711;
  ...
  _axios2.default.get("/api/stores/near?lat=" + lat + "&lng=" + lng).then(function (res) {
    ...
    //when someone clicks on the markers show a infowindow.
